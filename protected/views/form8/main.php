		...
		<div class="col-md-12">
			...
			<div class="portlet box blue">
				<div class="portlet-title">
					...
					<?php }else{?>
						<?php echo (UserLoginUtils::canCreate($_SERVER['REQUEST_URI']) == false)? "":  CHtml::link(ConfigUtil::getBtnAddName(),array('Form8/Create'),array('class'=>'btn btn-default btn-sm'));?>
					<?php }?>
					...
					<table class="table table-striped table-hover table-bordered"
						...
						<thead>
							...
							<th class="no-sort">ดำเนินการ</th>
								...
								<th>Rev.</th>
								<th>ชื่อชุดคณะกรรมการ</th>
								<!-- 								<th>รายละเอียดคำสั่งแต่งตั้ง</th> -->
								<!-- 								<th>ตำแหน่ง</th> -->
								<th>รายละเอียดคำสั่งแต่งตั้ง</th>
...
switch (UserLoginUtils::getUserRoleName ()) {
	case UserLoginUtils::ADMIN :
			if(UserLoginUtils::canDelete( $_SERVER['REQUEST_URI'])){?>
				...
				href="<?php echo Yii::app()->CreateUrl('Form8/Delete/id/'.$data->id)?>"></a>
			<?php }
		break;
}
		if ($data->status == "F") {
...
<a title="View" class="fa fa-search-minus"
									href="<?php echo Yii::app()->CreateUrl('Form8/View/id/'.$data->id)?>"></a>
<?php
		} else {
			switch (UserLoginUtils::getUserRoleName ()) {
				case UserLoginUtils::ADMIN :
				...
				<a title="แก้ไข" class="fa fa-edit"
									href="<?php echo Yii::app()->CreateUrl('Form8/Update/id/'.$data->id)?>"></a>
			<?php
						}
						if (UserLoginUtils::canDelete ( $_SERVER ['REQUEST_URI'] )) {
							?>
			...
			<?php
						...
						?>
			<a title="Commit"
									onclick="return confirm('ต้องการส่งข้อมูลให้ Staff ตรวจสอบ?')"
									class="fa fa-paper-plane"
									href="<?php echo Yii::app()->CreateUrl('Form8/ApproveStatus/id/'.$data->id.'/approve_status/'.UserLoginUtils::USER_APPROVE)?>"></a>
			<?php
					}
					if ($data->revision > 1) {
						?>
			<a title="ประวัติการแก้ไข" class="fa fa-clock-o"
									href="<?php echo Yii::app()->CreateUrl('Form8/Revision/id/'.$data->update_from_id)?>"></a>
		<?php
					}
				...
				case UserLoginUtils::STAFF :
					if ($data->approve_status == UserLoginUtils::USER_APPROVE) {
						...
						?>
			<a title="Commit"
									onclick="return confirm('ต้องการส่งข้อมูลให้  Executive ตรวจสอบ?')"
									class="fa fa-paper-plane"
									href="<?php echo Yii::app()->CreateUrl('Form8/ApproveStatus/id/'.$data->id.'/approve_status/'.UserLoginUtils::STAFF_APPROVE)?>"></a>
									<a title="Reject"
									onclick="return confirm('ส่งข้อมูลให้  User ตรวจสอบใหม่?')"
									class="fa fa-reply"
									href="<?php echo Yii::app()->CreateUrl('Form8/ApproveStatus/id/'.$data->id.'/approve_status/'.UserLoginUtils::INIT_APPROVE)?>"></a>
		...
		<?php
					if ($data->approve_status == UserLoginUtils::STAFF_APPROVE) {
						?>		
			<a title="Commit" onclick="return confirm('ยืนยันการบันทึกข้อมูล?')"
									class="fa fa-paper-plane"
									href="<?php echo Yii::app()->CreateUrl('Form8/ApproveStatus/id/'.$data->id.'/approve_status/'.UserLoginUtils::EXECUTIVE_APPROVE)?>"></a>
									<a title="Reject"
									onclick="return confirm('ส่งข้อมูลให้  Staff ตรวจสอบใหม่?')"
									class="fa fa-reply"
									href="<?php echo Yii::app()->CreateUrl('Form8/ApproveStatus/id/'.$data->id.'/approve_status/'.UserLoginUtils::USER_APPROVE)?>"></a>
...
<?php
		if( $data->no_committee_reason != ''){
			echo 'ไม่มีคณะกรรมการ  เนื่องจาก  (' . $data->no_committee_reason . ')';
		}else{
			echo $data->name;
		}
		...
		?>
								...
								<td class="center">
									...
									href="<?php echo ConfigUtil::getAppName().''. $data->file_path?>">&nbsp;ดาวโหลด</a>
									<?php }?>
									</td>
...
jQuery(document).ready(function () {
    ...
    $('#report_id').on('change', function() {
        if(this.value>0){
    		$("#btnDownload").attr("href", "Form8/Report"+this.value);
        }else{
